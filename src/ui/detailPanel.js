 ...
 * the main panel collapses/expands, or a briefing opens or closes. The site
 * briefing used to be appended here as an eighteenth section; it has its own
 * pane now (briefingPane.js), and takes this panel's slot while it is open.
 */
import { el } from './dom.js';

export function buildDetailPanel({ layers, order, controllers, onStateChange }) {
  ...
  /**
   * Show the panel only when it has something to say, the main panel is open,
   * AND no briefing is on screen.
   *
   * THE BRIEFING TAKES THIS PANEL'S SLOT. Three panes do not fit — 312 + 330 +
   * 330 ends at x=1014 and leaves 10px of map at 1024 wide — so while a briefing
   * is open this one yields rather than sitting beside it.
   *
   * That it comes BACK afterwards is not implemented anywhere, and deliberately
   * so: this expression is recomputed on every sync and drives the class,
   * aria-hidden and inert together, so there is no stored "previous state" that
   * could be restored wrongly. Both flags are passed in rather than read, so
   * there is one source of truth for each.
   */
  const sync = ({ collapsed, briefingOpen = false }) => {
    let any = false;
